...
const subtitleMap: Record<string, string> = {
  overview: 'Your family investment overview',
  transactions: 'View family transactions',
  members: 'Manage family members',
  portfolio: 'Family portfolio performance',
  goals: 'Track family savings goals',
  'ai-insights': 'AI-powered family insights',
...
export default function DemoFamilyDashboard() {
  const [activeTab, setActiveTab] = useState('overview')
  const [roundUpAmount, setRoundUpAmount] = useState(2)
  const [extraTx, setExtraTx] = useState<typeof familyTransactions>([])
  ...
  useEffect(() => {
    ...
    if (session) logDemoVisit(session.session_token, session.email, 'family')
  }, [])

  const allTransactions = useMemo(() => [...extraTx, ...familyTransactions], [extraTx])
  ...
  const handleSync = () => {
    ...
    setTimeout(() => {
      ...
      const newTx = createRandomTransactions('family', count, roundUpAmount)
  ...
  function renderContent() {
    switch (activeTab) {
      case 'overview':
        return <DemoOverviewTab transactions={allTransactions} holdings={familyHoldings} dashboardLabel="Family Portfolio" roundUpOverride={roundUpAmount} />
      case 'transactions':
        return <DemoTransactionsTab transactions={allTransactions} roundUpOverride={roundUpAmount} />
      case 'members':
        return <DemoMembersTab members={familyMembers} type="family" />
      case 'portfolio':
        return <DemoPortfolioTab holdings={familyHoldings} />
      case 'goals':
        return <DemoGoalsTab initialGoals={familyGoals} />
      case 'ai-insights':
        return <DemoAiInsightsTab transactions={allTransactions} roundUpOverride={roundUpAmount} />
      case 'notifications':
        return <DemoNotificationsTab />
      case 'settings':
        return <DemoSettingsTab profile={{ name: 'Carlos Martinez', email: 'carlos@demo.kamioi.com', phone: '(555) 987-6543', city: 'Miami', state: 'FL', roundUpAmount, plan: 'Family Plan' }} onRoundUpChange={setRoundUpAmount} />
      default:
        return <DemoOverviewTab transactions={allTransactions} holdings={familyHoldings} dashboardLabel="Family Portfolio" roundUpOverride={roundUpAmount} />
    }
  }
  ...
  return (
    <DashboardLayout
      ...
      sidebarUser={{ name: 'Martinez Family', role: 'Demo Family', initials: 'MF' }}
      greeting="Welcome, Martinez Family (Demo)"
      userInitials="MF"
    ...
    >
      ...
      <h1 className="aurora-page-title">
        {activeTab === 'overview' ? 'Family Dashboard' : navSections[0].items.find(i => i.id === activeTab)?.label || 'Dashboard'}
      </h1>
      <p className="aurora-page-subtitle">
        {subtitleMap[activeTab] || 'Your family investment overview'}
      </p>
      {renderContent()}
      <DemoReceiptModal
        open={receiptOpen}
