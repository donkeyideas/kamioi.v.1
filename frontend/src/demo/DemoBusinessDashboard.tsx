...
import { DemoMembersTab } from './DemoMembersTab'
import { DemoReportsTab } from './DemoReportsTab'
...
const navSections: NavSection[] = [
  {
    items: [
      ...
      { id: 'team', label: 'Team', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
      ...
      { id: 'reports', label: 'Reports', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> },
...
const subtitleMap: Record<string, string> = {
  overview: 'Company investment overview and activity',
  portfolio: 'Business portfolio performance',
  transactions: 'View business transactions',
  team: 'Manage team members',
  goals: 'Track business investment goals',
  'ai-insights': 'AI-powered business insights',
  analytics: 'Business spending and investment analytics',
...
export default function DemoBusinessDashboard() {
  ...
  const [roundUpAmount, setRoundUpAmount] = useState(3)
  const [extraTx, setExtraTx] = useState<typeof businessTransactions>([])
  ...
  useEffect(() => {
    ...
    if (session) logDemoVisit(session.session_token, session.email, 'business')
  }, [])

  const allTransactions = useMemo(() => [...extraTx, ...businessTransactions], [extraTx])
  ...
  const handleSync = () => {
    ...
    setTimeout(() => {
      ...
      const newTx = createRandomTransactions('business', count, roundUpAmount)
  ...
  function renderContent() {
    switch (activeTab) {
      case 'overview':
        return <DemoOverviewTab transactions={allTransactions} holdings={businessHoldings} dashboardLabel="Business Portfolio" roundUpOverride={roundUpAmount} />
      case 'portfolio':
        return <DemoPortfolioTab holdings={businessHoldings} />
      case 'transactions':
        return <DemoTransactionsTab transactions={allTransactions} roundUpOverride={roundUpAmount} />
      case 'team':
        return <DemoMembersTab members={businessTeam} type="business" />
      case 'goals':
        return <DemoGoalsTab initialGoals={businessGoals} />
      case 'ai-insights':
      ...
      case 'reports':
        return <DemoReportsTab />
      case 'notifications':
        return <DemoNotificationsTab />
      case 'settings':
        return <DemoSettingsTab profile={{ name: 'Jordan Blake', email: 'jordan@techstart.demo', phone: '(555) 456-7890', city: 'San Francisco', state: 'CA', roundUpAmount, plan: 'Business Enterprise' }} onRoundUpChange={setRoundUpAmount} />
      default:
        return <DemoOverviewTab transactions={allTransactions} holdings={businessHoldings} dashboardLabel="Business Portfolio" roundUpOverride={roundUpAmount} />
    }
  }
  ...
  return (
    <DashboardLayout
      ...
      sidebarUser={{ name: 'TechStart Inc.', role: 'Demo Business', initials: 'TS' }}
      greeting="Welcome, TechStart Inc. (Demo)"
      userInitials="TS"
    ...
    >
      ...
      <h1 className="aurora-page-title">
        {activeTab === 'overview' ? 'Business Dashboard' : navSections[0].items.find(i => i.id === activeTab)?.label || 'Dashboard'}
      </h1>
      <p className="aurora-page-subtitle">
        {subtitleMap[activeTab] || 'Company investment overview and activity'}
      </p>
      {renderContent()}
      <DemoReceiptModal
        open={receiptOpen}
